...
4a. **Recurrence check.** Ask: *"Could this class of bug happen again — is there a missing gate or outdated guideline that would prevent it?"*
   ...
   - **Yes**: propose prevention across one or more of:
     ...
     - A new truth in `taproot/global-truths/` capturing the convention or rule

   If a satisfactory measure is found: present it — e.g. *"I'll add `check-if-affected-by: <gate>` to `taproot/settings.yaml`"* or *"I'll add to `taproot/global-truths/<truth-file>.md`: `<convention>`"* — and wait for actor confirmation.
