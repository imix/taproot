...
> 💡 If this session is getting long, consider running `/compact` or starting a fresh context before the next task.
   ...
   **What's next?**
   [1] `/tr-implement <parent>/<slug>/` — start building
   [2] `/tr-audit <parent>/<slug>/usecase` — stress-test the spec first
   [3] `/tr-browse <parent>/<slug>/` — browse related behaviours and specs for context
...
- **What, not how.** Main flow steps must describe observable behaviour — what the actor sees or what the system produces — not the internal mechanism. "System saves the profile" is correct; "System executes an UPDATE query on the users table" is not. SQL, REST, HTTP verbs, endpoint names, and service internals belong in `impl.md`. The pre-commit hook flags implementation terms in the main flow.
