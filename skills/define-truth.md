...
- **No scope comments or top-level headings.** Do not add `<!-- Scope: impl -->` HTML comments — the filename suffix is authoritative. Do not add a `# Title` heading — the filename is the identifier. Start files directly with `## Section` headings.
...
- **Grouping over splitting.** When a split is needed, group related rules into sections within fewer files rather than creating one file per topic. 5-15 rules per file is the sweet spot. E.g. auth enforcement + validation + secrets = one file (related access-control concerns), not three separate files with 4 rules each.
