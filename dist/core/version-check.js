 ...
 */
export function checkProjectVersion(cwd) {
    ...
    const { version: installed } = require(resolve(__dirname, '..', '..', 'package.json'));
    ...
    if (instMaj < projMaj || (instMaj === projMaj && instMin < projMin)) {
        process.stderr.write(`taproot: project requires ${projectVersion}, installed is ${installed}\n` +
            `  → run: npm install -g @imix-js/taproot@${projectVersion}\n`);
    }
    else if (instMaj > projMaj) {
        process.stderr.write(`taproot: installed ${installed} is a major version ahead of this project (pinned at ${projectVersion})\n` +
            `  → run: taproot update  — to sync skills and record the new version\n`);
    }
    else if (instMin > projMin) {
        process.stderr.write(`taproot: installed ${installed}, project at ${projectVersion} — run: taproot update\n`);
    }
}
