...
export function registerAcceptanceCheck(program) {
    program
        ...
        if (testDirs.length === 0) {
            process.stderr.write('No test directory found — checked test/, tests/, spec/. Use --tests <dir> to specify.\n');
            process.exitCode = 1;
            return;
        }
...
export function runAcceptanceCheck(rootPath, testDirs) {
...
export function collectCriteria(rootPath) {
...
export function collectMissingSections(rootPath) {
    if (!existsSync(rootPath))
        return [];
    const tree = walkHierarchy(rootPath);
    const nodes = flattenTree(tree).filter(n => n.marker === 'behaviour');
...
export function scanTestFiles(testDirs) {
    const refs = [];
    for (const dir of testDirs) {
        if (existsSync(dir))
            walkTestDir(dir, refs);
    }
    return refs;
}
function walkTestDir(dir, refs) {
    let entries;
    try {
...
function scanFile(filePath, refs) {
    let content;
    try {
        content = readFileSync(filePath, 'utf-8');
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = AC_ID_IN_TEST.exec(line)) !== null) {
            if (m[1])
                refs.push({ id: `AC-${m[1]}`, file: filePath, line: i + 1 });
        }
    }
}
function printReport(report) {
    ...
    const parts = [];
    if (report.uncovered.length > 0)
        parts.push(`${report.uncovered.length} uncovered`);
    if (report.orphaned.length > 0)
        parts.push(`${report.orphaned.length} orphaned`);
    if (report.missingSections.length > 0)
        parts.push(`${report.missingSections.length} missing section`);
    process.stdout.write(parts.join(', ') + '\n');
}
//# sourceMappingURL=acceptance-check.js.map