...
function resolveSkillsRelPath(projectRoot, filename) {
...
export const ALL_AGENTS = ['claude', 'cursor', 'copilot', 'windsurf', 'gemini', 'generic', 'aider'];
...
function buildClaudeSkillFile(skill, projectRoot, cli) {
...
function generateCursorAdapter(skills, projectRoot, cli) {
...
function generateCopilotAdapter(skills, projectRoot, cli) {
...
function buildCopilotSection(skills, cli, projectRoot) {
    const skillSummary = skills.map(s => `- **\`/taproot:${s.name}\`** — ${s.description}. Full definition: \`${resolveSkillsRelPath(projectRoot ?? '', s.filename)}\``).join('\n');
...
function generateWindsurfAdapter(skills, projectRoot, cli) {
...
function buildWindsurfSection(skills, cli) {
    const skillIndex = skills.map(s => `- \`/taproot:${s.name}\` — ${s.description}`).join('\n');
    const skillSections = skills.map(s => `
### /taproot:${s.name}
...
function generateGeminiAdapter(skills, projectRoot, cli) {
...
function buildGeminiSkillFile(skill, projectRoot, cli) {
    const taprootBin = cli ?? 'taproot';
    const overviewStep = TREE_MODIFYING_SKILLS.has(skill.name)
...
function generateGenericAdapter(skills, projectRoot, cli) {
...
function buildGenericAgentsFile(skills, cli) {
    const skillIndex = skills.map(s => `- **\`/taproot:${s.name}\`** — ${s.description}`).join('\n');
    const skillSections = skills.map(s => `
---
...
function aiderReadPaths(skills, projectRoot) {
    return [
        ...skills.map(s => resolveSkillsRelPath(projectRoot, s.filename)),
        'CONVENTIONS.md',
    ];
}
export function generateAiderAdapter(skills, projectRoot) {
    const files = [];
    ...
    if (confExisted) {
        ...
        // Merge read: entries
        const existing = Array.isArray(parsed['read']) ? parsed['read'] : [];
...
function buildAiderConfYml(readPaths) {
...
function buildAiderConventionsMd(skills, projectRoot) {
    const skillList = skills.map(s => `- **${s.name}** — ${s.description}. Full definition: \`${resolveSkillsRelPath(projectRoot, s.filename)}\``).join('\n');
...
export function buildInvocationBlock(cli) {
    const prefix = cli ?? DEFAULT_CLI_PREFIX;
