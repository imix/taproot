...
describe('taproot update', () => {
  it('reports nothing to update when no adapters are installed', () => {
    const msgs = runUpdate({ cwd: tmpDir });
    expect(msgs[0]).toContain('No taproot agent adapters detected');
  });

  it('detects and regenerates claude adapter', () => {
  ...
  it('detects and regenerates cursor adapter', () => {
  ...
  it('removes stale .claude/skills/taproot/ directory', () => {
  ...
  it('removes stale tr-*.md files in .claude/skills/', () => {
  ...
  it('detects old taproot/ subdir layout as claude and migrates to commands/', () => {
    const staleDir = join(tmpDir, '.claude', 'skills', 'taproot');
    mkdirSync(staleDir, { recursive: true });
    writeFileSync(join(staleDir, 'intent.md'), 'old content');
  ...
  it('refreshes installed skills if present', () => {
    generateAdapters('cursor', tmpDir);
  ...
  it('ends with update complete message', () => {
