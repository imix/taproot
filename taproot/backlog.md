...
- [2026-04-12] incremental audit coverage — tr-audit-all is too large for big projects at any scale; needs scope × module filtering (e.g. /tr-audit-all frontend/ ux) and tracking of what was audited when (coverage matrix: path × module × last-audited date). Note: audit dimensions go beyond spec/code × security/ux/arch — could include docs completeness (are all features documented?), release audit (does changelog/docs cover all new features?), and others not yet imagined. Design should treat audit dimensions as open-ended, not a fixed enumeration. Extension: when the scoped audit is still too large for one session, offer to build a /tr-plan with one audit item per chunk — so the developer can work through it across sessions with progress tracking rather than starting over each time.
