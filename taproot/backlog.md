...
- [2026-04-01] document vertical slice in docs — explain the "vertical slice" concept: one intent → one behaviour → one impl, committed together, is the atomic unit of taproot delivery; this mental model is missing from docs/concepts.md and confuses new users
- [2026-04-01] /tr-bug step 2 UX issue: reproduction and location asked in one question — should be two separate questions; reproduction check should only trigger when relevant (clear symptoms like "I see X instead of Y" don't need a reproduction step — they're already confirmed)
