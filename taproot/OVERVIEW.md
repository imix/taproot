...
- **[hierarchy-sweep](./human-integration/hierarchy-sweep/usecase.md)** `[proposed]` — Actor: Developer — applying a uniform task across many hierarchy items without accumulating context drift. Also surfaced by `/tr-ineed` when the developer expresses a bulk-edit intent (e.g. "add X to all usecases").
...
- **[architecture-compliance](./implementation-quality/architecture-compliance/usecase.md)** `[specified]` — Actor: Developer or agent declaring a new implementation (committing an `impl.md`) — the check runs automatically at Definition of Ready time before any code is written.
...
---
15 intents · 34 behaviours · 34 implementations · 34/34 complete
