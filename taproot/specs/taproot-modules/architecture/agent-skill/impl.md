...
- `4542ed7173d9c706c5a435b74b0f08e4d22f8aac` — (auto-linked by taproot link-commits)
...
- condition: check-if-affected-by: skill-architecture/neutral-dod-resolution | note: NOT APPLICABLE — none of the 8 architecture module skills instruct the agent to run taproot dod or record DoD resolutions. arch-define.md wires DoD conditions (appends entries to settings.yaml) but does not run DoD evaluation itself. The skills reference /tr-commit in What's next blocks only, which is not the DoD resolution pattern this spec targets. | resolved: 2026-04-16T05:56:15.638Z

- condition: check-if-affected-by: taproot-modules/architecture | note: COMPLIANT — this impl IS the architecture module. The rework satisfies AC-12 (dep governance offer skipped when DoD already wired) and AC-13 (DoD wiring offer skipped when already present). All 7 aspects remain delivered. Naming, truth file writes, /tr-sweep offer, DoR/DoD wiring all intact. | resolved: 2026-04-16T05:55:58.291Z

- condition: check-if-affected-by: agent-integration/portable-output-patterns | note: COMPLIANT — unchanged from prior resolution. All 8 skill files use the [artifact-review] pattern declaration for presenting draft truth files before writing. The orchestrator uses a coverage table block for the scan output. Action prompts use canonical option-letter conventions. The rework only added guard text that produces no output (silent skip path). | resolved: 2026-04-16T05:55:53.675Z

- condition: check-if-affected-by: quality-gates/architecture-compliance | note: COMPLIANT — the rework modifies arch-define.md (a skill markdown file) and adds no new TypeScript CLI commands, no new core modules, no I/O boundary changes. The added guards read taproot/settings.yaml and conditionally skip offer steps. Follows the established module pattern. docs/architecture.md constraints respected. | resolved: 2026-04-16T05:55:46.653Z

- condition: check-if-affected-by: human-integration/pattern-hints | note: NOT APPLICABLE — unchanged from prior resolution. pattern-hints applies to skills that route natural-language requirement descriptions to matching patterns. The architecture module skills receive structured convention questions, not natural-language requirements. The rework did not change this. | resolved: 2026-04-16T05:55:42.089Z

- condition: check-if-affected-by: skill-architecture/commit-awareness | note: NOT APPLICABLE — unchanged from prior resolution. None of the 8 architecture module skills instruct the agent to run git commit. They reference /tr-commit in their What's next blocks as a suggested next step, which is the correct pattern. The rework did not change this. | resolved: 2026-04-16T05:55:38.600Z

- condition: check-if-affected-by: skill-architecture/context-engineering | note: COMPLIANT — unchanged from prior resolution. All skill descriptions are concise. The rework adds two short guard sentences (read one field from settings.yaml, skip if present). No embedded reference docs, no cross-skill repetition introduced. Session hygiene signal (/compact) remains in all 8 skills. | resolved: 2026-04-16T05:55:35.018Z

- condition: check-if-affected-by: agent-integration/agent-agnostic-language | note: COMPLIANT — unchanged from prior resolution. All 8 skill files use generic language: 'the agent', no 'Claude' or 'Claude Code' references, no agent-specific syntax. The rework only added read-then-skip guard text in plain markdown with no agent-specific terms. | resolved: 2026-04-16T05:55:30.122Z

- condition: check-if-affected-by: human-integration/pause-and-confirm | note: COMPLIANT — unchanged from prior resolution. The rework adds guards that skip steps silently when the DoD condition is already wired. No new multi-document writes without confirmation. The guard path is a no-op (skip), not a write action, so pause-and-confirm does not apply to those branches. | resolved: 2026-04-16T05:55:26.002Z

- condition: check-if-affected-by: human-integration/contextual-next-steps | note: COMPLIANT — unchanged from prior resolution. All 8 skill files (arch-define + 7 sub-skills) include a What's next block at the end of their primary output. The rework only added two read-then-skip guards in arch-define Phase 3 and Phase 5; no What's next blocks affected. | resolved: 2026-04-16T05:55:21.934Z

- condition: check: does this story reveal a reusable pattern worth documenting in docs/patterns.md? | note: NO new pattern introduced. The guard pattern (read settings.yaml before offering a DoD condition) follows the existing conditional-offer pattern already present in all quality module orchestrators. No new pattern type created. | resolved: 2026-04-16T05:55:16.537Z

- condition: check: does this story introduce a cross-cutting concern that warrants a new check-if-affected-by or check-if-affected entry in .taproot/settings.yaml? | note: NO — the rework adds settings.yaml read guards to arch-define.md. This is a fix to the architecture module's own orchestrator skill, not a new cross-cutting concern requiring a settings.yaml entry. | resolved: 2026-04-16T05:55:10.612Z

- condition: check: if this change modifies a skill file (skills/*.md), verify it does not introduce shell command execution without validation, does not hardcode credentials or tokens, and follows least-privilege for agent instructions — see docs/security.md | note: COMPLIANT — the rework modified arch-define.md only to add two read-then-skip guards (reading taproot/settings.yaml to check if a DoD condition is already present). No shell command execution introduced, no credentials or tokens, no new file writes beyond what was already reviewed. Least-privilege unchanged: skill only reads settings.yaml and decides whether to offer an option. | resolved: 2026-04-16T05:55:06.724Z
