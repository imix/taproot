...
- **audit-all integration is additive**: the truth discovery pass is added as a new final step in `audit-all.md`. It reuses the same scan and candidate logic, but outputs into a `## Truth Candidates` section in the review report and defers unprocessed candidates to backlog automatically.
- **One candidate at a time**: candidates are presented one at a time (not in batches) — the developer gives focused attention to each before moving on. Matches refined usecase AC-10/11/12.
- **Progress persistence via status file**: after each candidate action, the skill writes `.taproot/sessions/discover-truths-status.md`. On startup it offers [A] Resume / [R] Restart. Deleted on clean completion. Implements the batch-skill-progress global truth pattern.
...
- `0d86e1e505e9cfb6b2df15d9e6c808d766f471ac` — (auto-linked by taproot link-commits)
...
- condition: check: does this story introduce a cross-cutting concern that warrants a new check-if-affected-by or check-if-affected entry in .taproot/settings.yaml? | note: NO — progress persistence via .taproot/sessions/ is already captured as the batch-skill-progress global truth pattern; no new settings.yaml entry needed. | resolved: 2026-03-27T20:43:53.475Z
