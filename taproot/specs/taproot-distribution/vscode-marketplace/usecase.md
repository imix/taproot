...
- Extension source exists in `channels/vscode/` with a valid `package.json` manifest (name, publisher, version, keywords, icon, categories) — convention: all distribution channel artifacts live under `channels/<channel-name>/`
- `channels/vscode/package.json` version matches the taproot package version being released
- The `cut-release` CI workflow has been extended with a `publish-vscode-extension` job that runs after the `publish-npm` job succeeds, and a pre-flight check that verifies `channels/vscode/package.json` version matches the release version
- `channels/vscode/package.json` contains a resolved `publisher` value (not a placeholder), `displayName`, `description`, `categories`, keywords including "taproot" / "AI" / "requirements" / "BDD" / "spec" / "traceability", `icon`, a command contribution `taproot.init` with title "Taproot: Initialize project", and a walkthrough definition
- Publisher name in `channels/vscode/package.json` matches the registered VS Code Marketplace publisher account
- CI includes a README sync check: `channels/vscode/README.md` must not diverge from the root `README.md` content it is derived from — the `ci` job fails if `README.md` changed without a corresponding update to `channels/vscode/README.md`

## Main Flow

1. The `cut-release` CI `publish-vscode-extension` job starts — triggered automatically after the `publish-npm` job succeeds
2. CI builds the `.vsix` package: `cd channels/vscode && npx @vscode/vsce package`
...
- **Steps:**
  ...
  6. Maintainer runs the initial publish manually: `npx @vscode/vsce publish` and `npx ovsx publish` from the `channels/vscode/` directory
...
- **Trigger:** `channels/vscode/package.json` version does not match the taproot package version at release time
- **Steps:**
  ...
  2. CI job fails: "channels/vscode/package.json version (`X.Y.Z`) does not match release version (`A.B.C`). Update `channels/vscode/package.json` before releasing."
...
- **`vsce publish` fails — publisher name mismatch:** publisher field in `channels/vscode/package.json` does not match the registered Marketplace publisher; CI reports the mismatch and fails the step; maintainer corrects the publisher field and re-releases
...
- **`vsce package` fails — invalid manifest:** CI reports the manifest validation error and exits; maintainer fixes `channels/vscode/package.json` and re-releases
...
- **Channel README out of sync:** `scripts/sync-channel-readmes.sh --check` detects that root `README.md` changed without a corresponding update to `channels/vscode/README.md`; CI `ci` job fails with: "README.md changed — run `scripts/sync-channel-readmes.sh` to sync channel READMEs before pushing"
...
- Given `channels/vscode/package.json` contains a version different from the taproot release version
...
- Given the `.vsix` has been built from `channels/vscode/`
...
- Given root `README.md` has changed without `channels/vscode/README.md` being updated
- When the CI `ci` job runs the sync check
- Then the job fails with a message instructing the maintainer to run `scripts/sync-channel-readmes.sh`
...
- **Implementation phases**: two separable phases allow partial progress:
  - **Phase 1 (local — agent-completable):** icon at `channels/vscode/icon.png` (128×128, derived from `assets/logo-512.png`); `channels/vscode/package-lock.json` generated via `npm install`; release skill updated to bump both `package.json` versions; publisher placeholder resolved to `imix-ai`. These require no external accounts.
...
- **Shared brand assets:** `assets/` at project root holds the master icon set (`logo.svg`, `logo-96.png`, `logo-192.png`, `logo-512.png`, `favicon.ico`, `apple-touch-icon.png`). The VS Code extension icon (`channels/vscode/icon.png`) is a 128×128 copy derived from `assets/logo-512.png`.
