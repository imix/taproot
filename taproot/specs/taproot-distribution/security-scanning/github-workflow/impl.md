...
- **Tool version pinning:** semgrep pinned via pip (`==1.76.0`), syft and grype pinned via install scripts (`v0.104.0`, `v0.87.0`). Grype must be ≥ v0.82 for database schema v6 (earlier versions receive stale v5 databases that fail the 5-day age check). Semgrep must be ≥ v1.76.0 for Semgrep Registry rule access.
