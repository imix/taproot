...
import { loadLanguagePack, supportedLanguages, type LanguagePack } from '../core/language.js';
...
function isGlobalTruth(f: string): boolean {
...
function localizedHeading(englishKey: string, pack: LanguagePack | null): string {
...
export function checkIntentQuality(filePath: string, content: string, pack: LanguagePack | null = null): SpecFailure[] {
  const failures: SpecFailure[] = [];
  const goalHeading = localizedHeading('Goal', pack);
...
export async function runCommithook(options: { cwd: string }): Promise<number> {
  ...
  if (tiers.includes('requirement')) {
    ...
    for (const f of staged) {
      if (!isHierarchyFile(f) || isGlobalTruth(f) || isImplMd(f)) continue;
      // Only check actual spec files — not taproot/backlog.md, plan.md, settings.yaml, etc.
      if (!f.endsWith('intent.md') && !f.endsWith('usecase.md')) continue;
      const normalised = f.replace(/\\/g, '/');
      if (!normalised.startsWith(relRoot + '/') && normalised !== relRoot) {
        process.stdout.write(
          `taproot commithook — Requirement commit: hierarchy file outside configured root\n` +
          `  ✗ ${f}\n` +
    ...
    for (const f of staged) {
      if (!isHierarchyFile(f) || isGlobalTruth(f)) continue;
      const content = getStagedContent(f, cwd);
      if (!content) continue;
  ...
  if (tiers.includes('implementation')) {
    ...
    for (const [implFile] of implToSources) {
      ...
      if (!staged.includes(implFile)) {
        ...
        if (implState === 'complete' || implState === 'deferred') continue;
        process.stdout.write(
      ...
      if (getImplState(join(cwd, implFile)) === 'deferred') continue;
  ...
  if (globalTruthsDir(cwd)) {
    ...
    const linkedUnresolvable: string[] = [];
    const linkedOfflineSkipped: string[] = [];

    function collectTruthsForLevel(level: DocLevel): void {
  ...
  const linkWarnOnly = (linkConfig as unknown as Record<string, unknown>)['linkValidation'] === 'warn-only';
  const linkViolations = checkLinkTargets(
