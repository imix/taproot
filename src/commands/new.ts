...
const VALID_TYPES: ArtifactType[] = ['intent', 'behaviour', 'impl'];
...
function templateForType(type: ArtifactType, slug: string, parentRelPath?: string): { filename: string; content: string } {
...
export interface RunNewOptions {
  type: ArtifactType;
  parent?: string;  // required for behaviour and impl; omit for intent
  slug: string;
  cwd?: string;
}
...
export function runNew(options: RunNewOptions): NewResult {
  ...
  if (type === 'behaviour' || type === 'impl') {
    ...
    if (type === 'impl') {
      ...
      if (!hasUsecase) {
        if (hasIntent) {
          throw new Error(
            `Type mismatch: \`impl\` must be placed under a behaviour — \`${options.parent}\` contains an intent, not a behaviour`
          );
        }
  ...
  if (existsSync(targetFile)) {
    throw new Error(
      `Already exists: \`${targetFile}\` — open the file to edit it, or use /tr-behaviour or /tr-intent to refine it`
    );
  }
