...
export function runApply(options: {
  ...
  promptPath: string;
  cwd?: string;
  agentCli?: string; // injectable for tests
}): ApplyResult[] {
...
export function registerApply(program: Command): void {
  program
    ...
    .action((filelistPath: string, promptPath: string, options: { cwd?: string }) => {
