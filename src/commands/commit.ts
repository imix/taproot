...
function hasHierarchyFiles(staged: string[]): boolean {
  return staged.some(
    f =>
...
function hasGlobalTruths(cwd: string): boolean {
...
export function registerCommit(program: Command): void {
  program
    ...
    .action((message: string | undefined, options: { all?: boolean; dryRun?: boolean; cwd?: string }) => {
